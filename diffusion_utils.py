...
class SimpleDiffusion:
    def __init__(self, timesteps=200):
        ...
        self.initialize()
    def initialize(self):
        self.beta = jnp.linspace(10**(-4), 0.02, T)
...
def forward_process(key, sd:SimpleDiffusion, x_0):
    noise = random.normal(key, x_0.shape)
    reshaped_sqrt_alpha_bar_t = jnp.reshape(jnp.take(sd.sqrt_alpha_bar, sd.timesteps), (-1, 1))
    reshaped_sqrt_one_minus_alpha_bar_t = jnp.reshape(jnp.take(sd.sqrt_one_minus_alpha_bar, sd.timesteps), (-1, 1))
